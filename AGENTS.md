...
- Mark every Jira item that contains a Human gate with the `human-decision` label and a prominent `Human intervention gate` section. Add `human-action-required` only while a specific Human response or authorization is currently blocking progress, remove `agent-ready` while either `human-action-required` or `blocked` is present, and state the exact request and blocked transition in the issue.
- Before planning or starting a Jira item, and again before moving it to `In Review` or `Done`, scan its issue links, parent/version state, unresolved Human gates, and `blocked` or `human-action-required` labels. Do not cross a blocked transition. Record a Jira blocker-scan comment when a blocker is found or its state changes, and alert the user in the active conversation with the issue key, required action, and consequence.
